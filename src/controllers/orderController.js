...
const createOrder = async function (req, res) {
    try {
        ...
        if (!user) {
            return res.status(403).send({ status: false, message: "no user fou_id: cartId,nd" })
        }
        ...
        const cart = await cartModel.findOneAndUpdate({  totalPrice: { $ne: 0 } }, { $set: { items: [], totalItems: 0, totalPrice: 0 } }).lean()
        if (!cart) {
